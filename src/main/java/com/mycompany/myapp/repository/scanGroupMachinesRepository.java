...
public interface scanGroupMachinesRepository extends JpaRepository<scanGroupMachines, Long> {
    ...
    @Query(value = "insert into scan_group_machines (group_name,create_at,username,group_status) values(?1,?2,?3,?4) ", nativeQuery = true)
    public void insertGroupMachines(String groupName, ZonedDateTime createAt, String username, Integer groupStatus);

    @Query(
        value = "update scan_group_machines set group_name =?1 , update_at=?2,username =?3, group_status =?4 where group_id =?5",
        nativeQuery = true
    )
