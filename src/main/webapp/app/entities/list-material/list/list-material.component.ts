...
export class ListMaterialComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  columnSelectionGroup = signal<columnSelectionGroup>({
    ...
    subtasks: [
      ...
      { name: 'Calculated Status', matColumnDef: 'calculatedStatus', completed: false },
      ...
      { name: 'Available Quantity', matColumnDef: 'availableQuantity', completed: false },
      { name: 'Material TraceId', matColumnDef: 'materialTraceId', completed: false },
  ...
  ngOnInit(): void {
    ...
    this.materialService.selectedIds$.pipe(takeUntil(this.ngUnsubscribe)).subscribe(ids => {
    ...
    this.materialService.totalCount$.pipe(takeUntil(this.ngUnsubscribe)).subscribe(count => {
      this.length = count;
      this.cdr.markForCheck();
    });

    this.dataSource.filterPredicate = (data: RawGraphQLMaterial, filter: string): boolean => {
  ...
  updateSelectedItemsCount(): void {
    this.checkedCount.set(this.dataSource.data.filter(item => item.checked).length);
  }
  ...
  toggleAllRows(checked: boolean): void {
    ...
    if (checked) {
      const itemsToSelect = currentData.filter(row => !this.selection.isSelected(row));
    ...
    currentData.forEach(row => {
      const serviceKnowsSelected = row.checked;
      const modelWantsSelected = this.selection.isSelected(row);
  ...
  isDateField(colDef: string): boolean {
    const dateFields = ['expirationDate', 'receivedDate', 'updatedDate', 'checkinDate'];
  ...
  updateMasterCheckboxStatus(): void {
    ...
    const anyCompleted = currentGroup.subtasks!.some(subtask => subtask.completed);
