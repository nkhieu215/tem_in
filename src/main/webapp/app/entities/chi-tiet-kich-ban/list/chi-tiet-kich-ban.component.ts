...
export class ChiTietKichBanComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.chiTietKichBanService
      ...
      .subscribe({
        next: (res: HttpResponse<IChiTietKichBan[]>) => {
  ...
  delete(chiTietKichBan: IChiTietKichBan): void {
    const modalRef = this.modalService.open(ChiTietKichBanDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.chiTietKichBan = chiTietKichBan;
  ...
  protected onSuccess(data: IChiTietKichBan[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/chi-tiet-kich-ban'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
